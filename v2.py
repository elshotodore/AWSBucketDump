...
def write_interesting_file(filepath):
    try:
        ...
        with open('interesting_file_20171206.txt', 'ab+') as interesting_file:
...
def status200(response,grep_list,line):
    ...
    try:
        for child in objects['ListBucketResult']['Contents']:
            Keys['key'].append(child['Key'])
            Keys['size'].append(child['Size'])
            print(child['Key'] + "|" + child['Size'])
    except:
        pass
    hit = False
    for words in Keys["key"]:
